...
class _SfCalendarState extends State<SfCalendar> with SingleTickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    _calendarTheme = calendarThemeData.copyWith(
        ...
        timeIndicatorTextStyle: calendarThemeData.timeIndicatorTextStyle.copyWith(color: calendarThemeData.timeIndicatorTextStyle.color ?? themeData.colorScheme.secondary));
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      ...
      _isMobilePlatform = false; //CalendarViewHelper.isMobileLayout(Theme.of(context).platform);
      _useMobilePlatformUI = false; //CalendarViewHelper.isMobileLayoutUI(_minWidth, _isMobilePlatform);
  ...
  void _updateVisibleAppointments() async {
    ...
    if (_view != CalendarView.schedule) {
      ...
      if (_view == CalendarView.month &&
          !CalendarViewHelper.isLeadingAndTrailingDatesVisible(widget.monthViewSettings.numberOfWeeksInView, widget.monthViewSettings.showTrailingAndLeadingDates)) {
      ...
      final List<CalendarAppointment> tempVisibleAppointment =
          ...
          await AppointmentHelper.getVisibleAppointments(
              viewStartDate, viewEndDate, _appointments, widget.timeZone, _view == CalendarView.month || CalendarViewHelper.isTimelineView(_view));
      ...
      case CalendarDataSourceAction.add:
        {
          ...
          if (_view != CalendarView.schedule) {
            ...
            visibleAppointmentCollection.addAll(AppointmentHelper.getVisibleAppointments(
                viewStartDate, viewEndDate, collection, widget.timeZone, _view == CalendarView.month || CalendarViewHelper.isTimelineView(_view)));
          }
  ...
  DateTime _updateCurrentDate(CalendarView view) {
    ...
    if (_selectedDate != null && isDateWithInDateRange(visibleStartDate, visibleEndDate, _selectedDate)) {
      if (isMonthView) {
        return DateTime(
            _selectedDate!.year, _selectedDate!.month, _selectedDate!.day, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
      } else {
    ...
    for (int i = 0; i < allDayAppointmentView.length; i++) {
      ...
      if (intersectingAppointments.isNotEmpty) {
        final int maxPosition = intersectingAppointments
                .reduce((AppointmentView currentAppView, AppointmentView nextAppView) => currentAppView.position > nextAppView.position ? currentAppView : nextAppView)
                .position +
            1;
  ...
  void _updateMouseHoverPosition(Offset globalPosition, bool isScheduleDisplayDate, bool isRTL, DateTime? currentDate, double? startPosition,
  ...
  DateTime _getMaxAppointmentDate(
      List<CalendarAppointment> appointments, String? timeZone, DateTime maxDate, DateTime displayDate, ScheduleViewSettings scheduleViewSettings, bool useMobilePlatformUI) {
  ...
  DateTime _getMinAppointmentDate(
      List<CalendarAppointment> appointments, String? timeZone, DateTime minDate, DateTime displayDate, ScheduleViewSettings scheduleViewSettings, bool useMobilePlatformUI) {
  ...
  bool _isAppointmentBetweenDates(List<CalendarAppointment> appointments, DateTime startDate, DateTime endDate, String? timeZone) {
    ...
    for (int j = 0; j < appointments.length; j++) {
      ...
      final List<DateTime> recursiveDates = RecurrenceHelper.getRecurrenceDateTimeCollection(rule, appointment.actualStartTime,
          recurrenceDuration: AppointmentHelper.getDifference(appointment.actualStartTime, appointment.actualEndTime), specificStartDate: startDate, specificEndDate: endDate);
  ...
  Widget? _getItem(BuildContext context, int index, bool isRTL) {
    ...
    final List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(
        isSameOrAfterDate(_minDate!, startDate) ? startDate : _minDate!, isSameOrBeforeDate(_maxDate!, endDate) ? endDate : _maxDate!, _appointments, widget.timeZone, false,
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      void _addDisplayOrCurrentDateView({bool isDisplayDate = true}) {
        ...
        widgets.add(
            _getDisplayDateView(isRTL, isDisplayDate ? scheduleDisplayDate : scheduleCurrentDate, highlightViewStartPosition, viewPadding, appointmentViewHeaderHeight, padding));
      ...
      widgets.add(MouseRegion(
          ...
          child: GestureDetector(
            child: _ScheduleAppointmentView(
                header: Container(
                    child: CustomPaint(
                        painter: _AgendaDateTimePainter(currentDate, null, widget.scheduleViewSettings, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor,
                            widget.todayTextStyle, _locale, _calendarTheme, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform),
                ...
                content: Container(
                  ...
                  child: AgendaViewLayout(null, widget.scheduleViewSettings, currentDate, currentAppointments, isRTL, _locale, _localizations, _calendarTheme, _agendaViewNotifier,
                      widget.appointmentTimeTextFormat, viewPadding, _textScaleFactor, _isMobilePlatform, widget.appointmentBuilder, _minWidth - viewPadding, panelHeight, widget),
                )),
    ...
    if (_useMobilePlatformUI && isNeedInBetweenMonthBuilder && isNextMonthHasNoAppointment && isSameOrBeforeDate(_maxDate, endDate)) {
      ...
      scheduleViewDetails._intersectPoint =
          currentIndex >= 0 ? previousHeight + topHeight + appointmentHeight + viewTopPadding : previousHeight + height - topHeight - appointmentHeight - viewTopPadding;
  ...
  Widget _getMonthOrWeekHeader(DateTime startDate, DateTime? endDate, bool isRTL, bool isMonthLabel, {double viewPadding = 0, bool isNeedTopPadding = false}) {
    ...
    if (isMonthLabel && widget.scheduleViewMonthHeaderBuilder != null) {
      final ScheduleViewMonthHeaderDetails details =
          ScheduleViewMonthHeaderDetails(DateTime(startDate.year, startDate.month, 1), Rect.fromLTWH(0, 0, _minWidth, widget.scheduleViewSettings.monthHeaderSettings.height));
    ...
    return GestureDetector(
        child: Container(
            padding: isMonthLabel
                ...
                : EdgeInsets.fromLTRB(isRTL ? 0 : viewPadding, isNeedTopPadding ? padding : 0, isRTL ? viewPadding : 0, 0),
            child: RepaintBoundary(
                child: headerWidget != null
                    ...
                    : CustomPaint(
                        painter: _ScheduleLabelPainter(startDate, endDate, widget.scheduleViewSettings, isMonthLabel, isRTL, _locale, _useMobilePlatformUI, _agendaViewNotifier,
                            _calendarTheme, _localizations, _textScaleFactor),
                        size: isMonthLabel
                            ? Size(_minWidth, widget.scheduleViewSettings.monthHeaderSettings.height)
                            : Size(_minWidth - viewPadding - (2 * padding), widget.scheduleViewSettings.weekHeaderSettings.height),
                      ))),
        onTapUp: (TapUpDetails details) {
          _removeDatePicker();
          if (!CalendarViewHelper.shouldRaiseCalendarTapCallback(widget.onTap)) {
          ...
          CalendarViewHelper.raiseCalendarTapCallback(
              widget, DateTime(startDate.year, startDate.month, startDate.day), null, isMonthLabel ? CalendarElement.header : CalendarElement.viewHeader, null);
        },
        onLongPressStart: (LongPressStartDetails details) {
          _removeDatePicker();
          if (!CalendarViewHelper.shouldRaiseCalendarLongPressCallback(widget.onLongPress)) {
  ...
  Widget _getDisplayDateView(
      bool isRTL, DateTime currentDisplayDate, double highlightViewStartPosition, double viewHeaderWidth, double displayDateHighlightHeight, double padding) {
    return MouseRegion(
        onEnter: (PointerEnterEvent event) {
        ...
        child: GestureDetector(
          child: _ScheduleAppointmentView(
              header: Container(
                  child: CustomPaint(
                      painter: _AgendaDateTimePainter(currentDisplayDate, null, widget.scheduleViewSettings, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor,
                          widget.todayTextStyle, _locale, _calendarTheme, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform),
              ...
              content: Container(
                ...
                child: CustomPaint(
                    painter: _ScheduleLabelPainter(currentDisplayDate, null, widget.scheduleViewSettings, false, isRTL, _locale, _useMobilePlatformUI, _agendaViewNotifier,
                        _calendarTheme, _localizations, _textScaleFactor,
                        isDisplayDate: true),
  ...
  void _raiseCallbackForScheduleView(DateTime currentDate, Offset offset, List<CalendarAppointment> appointments, double viewHeaderWidth, double padding, bool isTapCallback,
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      List<CalendarAppointment> appointmentCollection =
          AppointmentHelper.getVisibleAppointments(viewStartDate, isSameOrBeforeDate(_maxDate!, viewEndDate) ? viewEndDate : _maxDate!, _appointments, widget.timeZone, false);
  ...
  Widget addAgendaWithLoadMore(double height, bool isRTL) {
    ...
    if (_nextDates.isNotEmpty &&
      ...
      while (isSameOrBeforeDate(_maxDate, viewStartDate)) {
        ...
        final List<CalendarAppointment> appointmentCollection =
            AppointmentHelper.getVisibleAppointments(appStartDate, appEndDate, _appointments, widget.timeZone, false, canCreateNewAppointment: false);
      ...
      while (viewStartDate.isBefore(scheduleDisplayDate) && !isSameDate(viewStartDate, scheduleDisplayDate)) {
        ...
        double todayNewEventHeight =
            !isSameDate(scheduleCurrentDate, scheduleDisplayDate) && isDateWithInDateRange(appStartDate, appEndDate, scheduleCurrentDate) ? displayEventHeight : 0;
        final List<CalendarAppointment> appointmentCollection =
            AppointmentHelper.getVisibleAppointments(appStartDate, appEndDate, _appointments, widget.timeZone, false, canCreateNewAppointment: false);
    ...
    if ((_isNeedLoadMore || _isScheduleStartLoadMore) && widget.loadMoreWidgetBuilder != null) {
      final Alignment loadMoreAlignment =
          _agendaScrollController!.hasClients && _agendaScrollController!.position.pixels <= _agendaScrollController!.position.minScrollExtent && _isScheduleStartLoadMore
              ? Alignment.topCenter
  ...
  Widget _getCalendarViewPopup() {
    ...
    final Color? todayColor =
        CalendarViewHelper.getTodayHighlightTextColor(widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _calendarTheme);
    ...
    final double calendarViewWidth = _useMobilePlatformUI
        ? iconWidth
        : _getTextWidgetWidth(calendarViews[_view]!, widget.headerHeight, _minWidth - totalArrowWidth, context, style: style).width + padding + headerIconTextWidth;
        ...
        child: _PopupWidget(
            ...
            child: Container(
                ...
                child: Material(
                  ...
                  child: ListView(padding: const EdgeInsets.all(0), controller: ScrollController(initialScrollOffset: scrollPosition), children: children),
                ))));
  }
              ...
              child: MouseRegion(
                  ...
                  child: GestureDetector(
                    child: ListView(
                        ...
                        children: <Widget>[
                          ResourceViewWidget(_resourceCollection, widget.resourceViewSettings, resourceItemHeight, widget.cellBorderColor, _calendarTheme, _resourceImageNotifier,
                              isRTL, _textScaleFactor, _resourceHoverNotifier.value, _imagePainterCollection, resourceViewSize, panelHeight, widget.resourceViewHeaderBuilder),
                        ]),
                    onTapUp: (TapUpDetails details) {
                ...
                child: SfDateRangePicker(
                  ...
                  onViewChanged: (DateRangePickerViewChangedArgs details) {
                    ...
                    _controller.displayDate = DateTime(
                        selectedDate.year, selectedDate.month, selectedDate.day, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
                  },
                  onSelectionChanged: (DateRangePickerSelectionChangedArgs details) {
                    ...
                    _showHeader = false;
                    _controller.displayDate = DateTime(details.value.year, details.value.month, details.value.day, _controller.displayDate!.hour, _controller.displayDate!.minute,
  ...
  void _handleTapForAgenda(TapUpDetails details, DateTime? selectedDate) {
    ...
    CalendarViewHelper.raiseCalendarTapCallback(
        widget, selectedDate, selectedAppointments, selectedAppointments.isNotEmpty ? CalendarElement.appointment : CalendarElement.agenda, null);
  }
  ...
  Widget _addAgendaView(double height, double startPosition, double width, bool isRTL) {
    ...
    if (_selectedDate != null) {
      currentSelectedDate = isDateWithInDateRange(widget.minDate, widget.maxDate, _selectedDate!) && !CalendarViewHelper.isDateInDateCollection(_blackoutDates, _selectedDate!)
          ? _selectedDate
          : null;
    }
      ...
      return Positioned(
          ...
          child: _OpacityWidget(
              ...
              child: Container(
                  ...
                  child: GestureDetector(
                    child: AgendaViewLayout(widget.monthViewSettings, null, currentSelectedDate, null, isRTL, _locale, _localizations, _calendarTheme, _agendaViewNotifier,
                        widget.appointmentTimeTextFormat, 0, _textScaleFactor, _isMobilePlatform, widget.appointmentBuilder, width, height, widget),
                    onTapUp: (TapUpDetails details) {
            ...
            child: Container(
                ...
                child: MouseRegion(
                    ...
                    child: GestureDetector(
                      child: Stack(children: <Widget>[
                        CustomPaint(
                          painter: _AgendaDateTimePainter(currentSelectedDate, widget.monthViewSettings, null, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor,
                              widget.todayTextStyle, _locale, _calendarTheme, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform),
...
class _CalendarHeaderViewState extends State<_CalendarHeaderView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showDatePickerButton) {
      ...
      if (!useMobilePlatformUI) {
        ...
        final Size todayButtonSize =
            _getTextWidgetWidth(todayText, widget.height, widget.width - totalArrowWidth, context, style: const TextStyle(fontSize: defaultCalendarViewTextSize));
    ...
    if (!widget.isMobilePlatform || (widget.isMobilePlatform && weekNumberEnabled)) {
      final Size headerTextSize = _getTextWidgetWidth(headerString, widget.height, widget.width - totalArrowWidth - todayIconWidth - padding, context,
    ...
    final Color? highlightColor =
        CalendarViewHelper.getTodayHighlightTextColor(widget.todayHighlightColor ?? widget.calendarTheme.todayHighlightColor, widget.todayTextStyle, widget.calendarTheme);
    ...
    if (isNeedViewSwitchOption) {
      ...
      if (useMobilePlatformUI) {
        ...
        for (int i = 0; i < allowedViewsLength; i++) {
          ...
          final Size calendarViewSize =
              _getTextWidgetWidth(_calendarViews[currentView]!, widget.height, totalWidth, context, style: const TextStyle(fontSize: defaultCalendarViewTextSize));
        ...
        if (allowedViewsWidth < totalWidth) {
          ...
          for (int i = 0; i < allowedViewsLength; i++) {
            final CalendarView currentView = widget.allowedViews![i];
            children.add(_getCalendarViewWidget(headerBackgroundColor, useMobilePlatformUI, false, calendarViewsWidth[currentView]!, maxHeaderHeight, style, arrowColor,
                headerTextColor, currentView, widget.view == currentView, highlightColor, defaultCalendarViewTextSize));
          }
        } else {
          /// Render allowed views drop down when header view does not have a
          /// space to hold the allowed views.
          final Size calendarViewSize = _getTextWidgetWidth(_calendarViews[widget.view]!, widget.height, widget.width - totalArrowWidth, context,
          ...
          children.add(_getCalendarViewWidget(headerBackgroundColor, useMobilePlatformUI, true, calendarViewWidth, maxHeaderHeight, style, arrowColor, headerTextColor, widget.view,
              widget.viewChangeNotifier.value, highlightColor, defaultCalendarViewTextSize,
              semanticLabel: 'CalendarView'));
        }
      }
    }
    ...
    if (weekNumberEnabled) {
      ...
      final Size weekNumberPanelSize =
          _getTextWidgetWidth(widget.localizations.weeknumberLabel + weekNumberString + ' ', headerHeight, weekNumberPanelWidth, context, style: weekNumberTextStyle);
                      ...
                      child: Row(
                          ...
                          children: widget.showDatePickerButton
                              ? <Widget>[
                                  Flexible(
                                      child: Text(headerString,
                                          ...
                                          maxLines: 1,
                                          semanticsLabel: headerString + ' ' + (widget.isPickerShown ? 'hide date picker' : 'show date picker'),
                              ...
                              : <Widget>[
                                  Flexible(
                                      child:
                                          Text(headerString, style: headerTextStyle, maxLines: 1, overflow: TextOverflow.clip, softWrap: false, textDirection: TextDirection.ltr))
                                ])),
                )),
          )
        : Container(
                  ...
                  child: Container(
                      ...
                      child: Row(
                        ...
                        children: widget.showDatePickerButton
                            ...
                            : <Widget>[
                                Flexible(
                                    child: Text(headerString, style: headerTextStyle, maxLines: 1, overflow: TextOverflow.clip, softWrap: false, textDirection: TextDirection.ltr)),
                              ],
                      )),
                )),
          );
  ...
  Widget _getCalendarViewWidget(Color headerBackgroundColor, bool useMobilePlatformUI, bool isNeedIcon, double width, double height, TextStyle style, Color arrowColor,
      Color headerTextColor, CalendarView view, bool isHighlighted, Color? highlightColor, double defaultCalendarViewTextSize,
...
class _ScheduleLabelPainter extends CustomPainter {
  _ScheduleLabelPainter(this.startDate, this.endDate, this.scheduleViewSettings, this.isMonthLabel, this.isRTL, this.locale, this.useMobilePlatformUI, this.agendaViewNotifier,
...
class _AgendaDateTimePainter extends CustomPainter {
  _AgendaDateTimePainter(this.selectedDate, this.monthViewSettings, this.scheduleViewSettings, this.todayHighlightColor, this.todayTextStyle, this.locale, this.calendarTheme,
      this.agendaDateNotifier, this.viewWidth, this.isRTL, this.textScaleFactor, this.isMobilePlatform)
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (monthViewSettings != null) {
      ...
      dayTextStyle = scheduleViewSettings!.dayHeaderSettings.dayTextStyle ??
          (useMobilePlatformUI
              ...
              : TextStyle(color: calendarTheme.agendaDayTextStyle.color, fontSize: 9, fontFamily: 'Roboto', fontWeight: FontWeight.w500));
      dateTextStyle = scheduleViewSettings!.dayHeaderSettings.dateTextStyle ??
          (useMobilePlatformUI
              ? calendarTheme.agendaDateTextStyle
              : TextStyle(color: calendarTheme.agendaDateTextStyle.color, fontSize: 18, fontFamily: 'Roboto', fontWeight: FontWeight.normal));
    }
    ...
    if (isToday) {
      ...
      dateTextStyle =
          todayTextStyle != null ? todayTextStyle!.copyWith(fontSize: dateTextStyle.fontSize, color: todayTextStyleColor) : dateTextStyle.copyWith(color: todayTextStyleColor);
    }
  ...
  void _addDayLabelForWeb(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday) {
    ...
    final String dayText = DateFormat(isRTL ? scheduleViewSettings!.dayHeaderSettings.dayFormat + ', MMM' : 'MMM, ' + scheduleViewSettings!.dayHeaderSettings.dayFormat, locale)
        .format(selectedDate!)
        .toUpperCase()
        .toString();
